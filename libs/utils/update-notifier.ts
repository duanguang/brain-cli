  ...
  if (
    ...
    notifier.notify({
      message: `${chalk
        .hex('#24E488')
        .bold(
          `${update.name}: New version found`
        )} \n update available ${chalk.hex('#909196')(
        update.current
      )} → ${chalk.green(update.latest)} \n Run ${chalk.hex('#88DFEF')(
        `yarn add ${update.name}`
      )} to update
                `,
      defer: false,
      //@ts-ignore
...
export const chkUpdateNotifier = () => {

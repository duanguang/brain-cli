 ...
 */
function withReactRefresh(babelOptions: any) {
  return {
    ...babelOptions,
    plugins: [
...
export const getTsLoadersed = (include:string[]=[], reactRefresh: boolean = false) => {
  const loaders = [];
...
export const getJSXLoadersed = (include:string[]=[], reactRefresh: boolean = false) => {
    const loaders = [];
    const hotLoader = [];
